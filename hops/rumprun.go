...
func (i *rumprunInfo) SupportsFsType(fsType string) bool {
	switch fsType {
	case "ext2":
		return true
	default:
...
func (i *rumprunInfo) SupportsMonitor(mon string) bool {
	switch mon {
	case "hvt":
		return true
	case "spt":
...
func (i *rumprunInfo) UpdateRootfs(buildContext string) (llb.State, error) {
	local := llb.Local(buildContext)
	base := llb.Image(i.Rootfs.From)
	return FilesLLB(i.Rootfs.Includes, local, base, -1)
}
