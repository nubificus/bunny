...
func (i *mirageInfo) CreateRootfs(buildContext string) llb.State {
	return llb.Scratch()
}
...
func (i *mirageInfo) BuildKernel(buildContext string) llb.State {
	...
	workState := FilesLLB([]string{"/:"+"/home/opam" + workDir}, content, toolSet, 1000)
	var envMode string
	if i.Monitor == "qemu" {
		envMode = "virtio"
	} else {
	...
	confExec := workState.Dir("/home/opam"+workDir).
		User("opam").
		AddEnv("CAML_LD_LIBRARY_PATH", "/home/opam/.opam/4.14/lib/stublibs:/home/opam/.opam/4.14/lib/ocaml/stublibs:/home/opam/.opam/4.14/lib/ocaml").
		AddEnv("OCAML_TOPLEVEL_PATH", "/home/opam/.opam/4.14/lib/toplevel").
		AddEnv("OPAMYES", "1").
		AddEnv("OPAMPRECISETRACKING", "1").
		AddEnv("OPAMERRLOGLEN", "0").
		AddEnv("OPAM_SWITCH_PREFIX", "/home/opam/.opam/4.14").
		AddEnv("OPAMCONFIRMLEVEL", "unsafe-yes").
		AddEnv("PATH", "/home/opam/.opam/4.14/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin").
		AddEnv("MODE", envMode).
